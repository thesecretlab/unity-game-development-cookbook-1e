...
// normally be avoided. However, saving the game happens comparitively
// rarely, and is expected to be a moderately heavy operation anyway (that
// is, players expect the game to pause for a moment.) Additionally, by
// calling System.GC.Collect() when we're done, we do much of the cleanup
// that would otherwise happen at some unpredictable amount of time in the
// future.
...
public static class SavingService
{
    ...
    // strings we use to store and look up items in the JSON as constant
    // strings
    ...
    // (use an unexpected character '$' for the Save ID here to reduce the
    ...
    public static void SaveGame(string fileName) {
        ...
        // Find all MonoBehaviours by first finding every single
        // MonoBehaviour, and filtering it to only include those that are
        // ISaveable.
        ...
        if (allSaveableObjects.Count() > 0) {
            ...
            foreach (var saveableObject in allSaveableObjects)
            {
                ...
                // dictionary) because we need to include the object's Save
                // ID
                if (data.IsObject) {
        ...
        // Ask the scene manager how many scenes are open, and for each
        // one, store the scene's name.
    ...
    public static bool LoadGame(string fileName) {
        ...
        // Ensure that we successfully read the data, and that it's an
        // object (i.e. a JSON dictionary)
        ...
        for (int i = 0; i < sceneCount; i++)
        {
            ...
            // If this is the first scene we're loading, load it and
            // replace every other active scene.
            if (i == 0)
            {
        ...
        if (data.ContainsKey(OBJECTS_KEY)) {
            ...
            // We cannot update the state of the objects right away,
            // because Unity will not complete loading the scene until some
            // time in the future. Any changes we made to the objects would
            // be reverted back to how they're defined in the original
            // scene. As a result, we need to run the code after the scene
            // manager reports that a scene has finished loading.

            // To do this, we create a new delegate that contains our
            // object-loading code, and store that in
            // LoadObjectsAfterSceneLoad. This delegate is added to the
            // SceneManager's sceneLoaded event, which makes it run after
            // the scene has finished loading.

            LoadObjectsAfterSceneLoad = (scene, loadSceneMode) => {

                // Find all ISaveable objects, and build a dictionary that
                // maps their  Save IDs to the object (so that we can
                ...
                // And ask the garbage collector to tidy up (again, this
                // will cause a performance hitch, but users are fine with
                // this since they're already waiting for the scene to
                // finish loading)
                System.GC.Collect();
            };

            // Register the object-loading code to run after the scene
            // loads.
            SceneManager.sceneLoaded += LoadObjectsAfterSceneLoad;
        }
