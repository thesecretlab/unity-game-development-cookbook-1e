...
// stores information about the position, rotation and scale of the game object
// that it's attached to.

// To use this, add a TransformSaver component to your game object. It will 
// automatically be included in saved games, and its state will be restored when
// the game is loaded.
...
public class TransformSaver : SaveableBehaviour
{
    ...
    // SerializeValue is a helper function that converts an object that Unity
    // already knows how to serialize (like Vector3, Quaternion, and others)
    ...
    private JsonData SerializeValue(object obj) {
        ...
        // but it means that we don't need to write the (de)serialization
        // code for built-in Unity types
        return JsonMapper.ToObject(JsonUtility.ToJson(obj));
    }

    // DeserializeValue works in reverse - given a JsonData, it produces a value
    // of the desired type, as long as that type is one that Unity already knows
    // how to serialize.
    ...
    public override JsonData SavedData {
        get {
            // Create the JsonData that we'll return to the saved game system
    ...
    {
        // We can't assume that the data will contain every piece of data that
        // we store; remember the programmer's adage, "be strict in what you
        // generate, and forgiving in what you accept".
